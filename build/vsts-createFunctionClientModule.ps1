...
$parentModule = 'SecretManagement.NetwrixPasswordSecure'
if (-not $ModuleName) { $ModuleName = 'SecretManagement.NetwrixPasswordSecure.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\SecretManagement.NetwrixPasswordSecure'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\SecretManagement.NetwrixPasswordSecure.psd1"
$prereqHash = @{
