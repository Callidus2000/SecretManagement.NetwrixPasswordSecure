...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'SecretManagement.NetwrixPasswordSecure.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'SecretManagement.NetwrixPasswordSecure.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'SecretManagement.NetwrixPasswordSecure.Client.ProtectedToken'
